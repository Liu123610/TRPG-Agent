...
class EnhancedMemory:
    ...
    def _create_enhanced_summary(self, old_messages: List[AnyMessage]) -> SystemMessage:
        ...
        # 过滤掉错误消息，提高摘要质量
        valid_messages = [msg for msg in old_messages
                         if not (isinstance(msg, AIMessage) and
                               str(msg.content).startswith('[AI服务'))]

        # 如果没有有效消息，返回简单摘要
        if not valid_messages:
            error_count = sum(1 for msg in old_messages
                            if isinstance(msg, AIMessage) and
                            str(msg.content).startswith('[AI服务'))
            return SystemMessage(content=f"[之前的对话因{error_count}次AI服务错误已压缩]")

        # 消息类型分析（基于有效消息）
        type_stats = self._analyze_message_types(valid_messages)

        # 关键信息提取（基于有效消息）
        key_info = self._extract_key_info(valid_messages)

        # 工具调用统计（基于有效消息）
        tool_stats = self._analyze_tool_calls(valid_messages)

        # 生成结构化摘要
        summary_text = self._build_structured_summary(type_stats, key_info, tool_stats, valid_messages)
    ...
    def _extract_key_info(self, messages: List[AnyMessage]) -> List[str]:
        ...
        for msg in messages:
            if isinstance(msg, HumanMessage):
                ...
                # 识别TRPG相关的重要表达
                trpg_keywords = ['职业', '法术', '冒险', '角色', '酒馆', '任务', '村子']
                important_keywords = ['决定', '选择', '要', '想', '必须', '重要', '记住']
                preference_keywords = ['喜欢', '讨厌', '偏好', '习惯', '风格']

                # 优先提取TRPG相关内容
                if any(keyword in content for keyword in trpg_keywords):
                    if len(content) > 60:
                        content = content[:57] + "..."
                    key_info.append(f"游戏: {content}")
                elif any(keyword in content for keyword in important_keywords):
                    if len(content) > 50:
                        content = content[:47] + "..."
                    key_info.append(f"用户: {content}")
                elif any(keyword in content for keyword in preference_keywords):
                    if len(content) > 50:
                        content = content[:47] + "..."
                    key_info.append(f"偏好: {content}")

        return key_info[:3]  # 减少到最多3条关键信息，提高质量

    def _extract_content_from_message(self, msg: AnyMessage) -> str:
        """从消息中提取文本内容，处理多种内容类型"""
    ...
    def _build_structured_summary(self, type_stats: Dict[str, int], key_info: List[str],
        ...
        if key_info:
            ...
            for info in key_info:  # 已经限制为最多3条
                summary_parts.append(f"  - {info}")

        # 4. 游戏进度信息（如果适用）
        trpg_info = [info for info in key_info if info.startswith('游戏:')]
        if trpg_info:
            summary_parts.append(f"\n游戏进度:")
            summary_parts.append(f"  继续TRPG冒险，当前关注: {len(trpg_info)}个游戏相关决策")
